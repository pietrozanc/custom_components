...
static const char *TAG = "pzem004t2";
  ...
  while (this->available() >= 7) {
    ...
    switch (resp[0]) {
      ...
      case 0xA0: {  // Voltage Response
        uint16_t int_voltage = (uint32_t(resp[1]) << 8) | (uint16_t(resp[2]) << 0);
	  ...
	  case 0xA3: { // Energy Response
        uint32_t energy = (uint16_t(resp[1]) << 16) | (uint16_t(resp[2]) << 8) | (uint16_t(resp[3]) << 0);
